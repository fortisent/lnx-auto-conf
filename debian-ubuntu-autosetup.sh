...
install_packages_stage() {
  local package
  local -a requested=(openssh-server sudo ufw traceroute nano wget git fail2ban clamav clamav-freshclam cron ca-certificates curl dnsutils iputils-ping netcat-openbsd unattended-upgrades)
...
print_results() {
  ...
  printf 'Журнал: %s\n' "$LOG_FILE"
  [[ -n "$BACKUP_DIR" ]] && printf 'Резервные копии: %s\n' "$BACKUP_DIR"
}
